...
cd $CURRENT_DIR
MODULEDIR=$UBCESLAB_SWENV_PREFIX/apps/lmod/modulefiles/asciidoc
mkdir -p $MODULEDIR

echo "local version = \"$ASCIIDOC_VERSION\"" > $MODULEDIR/$ASCIIDOC_VERSION.lua
echo "local apps_dir = \"$UBCESLAB_SWENV_PREFIX/apps\"" >> $MODULEDIR/$ASCIIDOC_VERSION.lua
cat ../modulefiles/asciidoc.lua >> $MODULEDIR/$ASCIIDOC_VERSION
