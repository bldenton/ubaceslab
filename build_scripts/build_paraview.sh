...
if [ ! -f paraview-$PARAVIEW_VERSION.tar.gz ]; then
  wget  --output-document=paraview-$PARAVIEW_VERSION.tar.gz "https://www.paraview.org/paraview-downloads/download.php?submit=Download&version=v5.5&type=binary&os=Linux&downloadFile=ParaView-5.5.2-Qt5-MPI-Linux-64bit.tar.gz"
...
mv ParaView-$PARAVIEW_VERSION-Qt5-MPI-Linux-64bit $PARAVIEW_DIR
